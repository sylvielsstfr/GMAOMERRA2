...
ax.set_ylabel('M=2.5*log10(atm. transparency)')
ax.set_ylim(-1.,0.)
cut_idx=50


transp_curv, = ax.plot(WL[cut_idx:], 2.5*np.log10(transp[selected_indexes[0],cut_idx:]), 'r-', linewidth=2)
...
def update(i):
    ...
    transp_curv.set_ydata(2.5*np.log10(transp[sel_index,cut_idx:]))
    ax.set_xlabel(label)
